...
export default async function ExpenseDetailPage({ params }: Params) {
  ...
  const { data, error } = await supabase.from("expenses").select("*, attachment_urls").eq("id", params.id).single()
  if (error) {
    return (
    ...
    <div className="container mx-auto p-6">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Details for expense ID: {expense.id}</p>
        </div>
      ...
      <div className="bg-white rounded shadow p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3">
          {Object.entries(expense)
            .filter(([key, value]) => value !== null && value !== undefined && value !== "" && value !== false && key !== "id" && key !== "user_id" && key !== "category_id")
            .map(([key, value]) => (
          ...
          {Array.isArray(expense.attachment_urls) && expense.attachment_urls.length > 0 && (
            <div className="flex flex-col gap-1 border-b pb-2 last:border-b-0">
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {expense.attachment_urls.map((url: string, idx: number) => {
