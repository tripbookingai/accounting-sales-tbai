...
// Type for inserting a new sale that aligns exactly with what the helper function expects.
...
export async function POST(request: Request) {
  try {
    ...
    const saleData: InsertSale = {
      ...
      sale_amount: Number(body.sale_amount) || 0,
      cogs: Number(body.cogs) || 0,
      transaction_fee_percent: Number(body.transaction_fee_percent) || 0,
      ...
      // Auto-calculated fields usually generated by the DB (set to 0 safely, DB overwrites it if setup as 'generated stored')
      // Note: If they aren't generated columns but rather computed in Next.js, you'll need to do the math here:
...
export async function PATCH(request: Request) {
  try {
    ...
    // We do NOT need to calculate or update `nights` because the database generates `nights` automatically based on `checkin_date`, `checkout_date`, and `number_of_rooms`
